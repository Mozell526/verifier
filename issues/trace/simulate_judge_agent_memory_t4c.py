"""In-memory T4c mouth. Official files are not written.

T4c keeps the same two questions as T4. It removes the example locutions
「查一下 / 帮我找」and the phrasing 「语气 / 修饰」. Q2 is only:

    drop this part of the utterance — does the success criterion change?

Writes only to simulate_judge_agent_memory.t4c-extra.json.
...
OUT = HERE / "simulate_judge_agent_memory.t4c-extra.json"
LIVE = HERE / "simulate_judge_agent_memory.json"
FROZEN_T4 = HERE / "simulate_judge_agent_memory.t4.json"

T4C_TREATMENT = "generic_two_question_asked_to_deliver_q1_evidence_shortcircuit_disabled"
...
办成的标准是：原句要求交出来的条件，这次是否都有对应交付。
不是“检索结果集会不会因此变”，也不是“有没有写出具体值”。
问句指定了要找或要交的东西，即使这次没有写出具体值，也算还要了；没有对应条件就是没说清。
问句里只是在提出请求、没有指定要找或要交的东西，不是另一件要办的事。

不要为了迁就已经交出来的条件，把原句还要交的那一部分丢掉。
没有另一件没被交代的事：再看第一问。
...
def _patch() -> None:
    ...
    def active(treatment: str) -> str:
        if treatment == T4C_TREATMENT:
            return PRINCIPLE_T4C
...
def _score(rows: list[dict]) -> dict:
    ...
    must_fail = []
    observe = []
    for case_id, query, _pairs, policy in T4C_CASES:
...
def main() -> None:
    ...
    runs = sim.load_runs()
    cases = []
    for case_id, query, pairs, _policy in T4C_CASES:
        cases.append(
            {
                "id": case_id,
                "query": query,
    ...
    payload = {
        "note": (
            "T4c in-memory abstraction probe. Not the frozen T4 12-row score. "
            "Official files were not written."
        ),
        "principle": PRINCIPLE_T4C,
        "treatment": T4C_TREATMENT,
        "banned_in_principle": [
            token
            for token in ["查一下", "帮我找", "语气", "修饰", "这一维", "2–4", "有姓", "姓名题", "inherit"]
            if token in PRINCIPLE_T4C
        ],
    ...
    if LIVE.is_file() and LIVE.read_bytes() != live_before:
        raise SystemExit("T4c wrote the live dump; abort and restore from backup")
    if FROZEN_T4.is_file() and FROZEN_T4.read_bytes() != t4_before:
        raise SystemExit("T4c wrote the frozen T4 dump; abort")
