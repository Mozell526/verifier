...
- /Users/xiaozijian/WorkSpace/projects/claude_code/verifier-runB/verifier/issues/open/issue-073.md

Do **not** review or append to issue-006 through issue-069. Those belong to other charters and already have their own status. You may cite Consensus blocks only from 015, 046, 047, 048, 058, 060, 061, 062, 063, 064, 065.

Do **not** mix this with issue-053 through issue-057, or with issue-066 (T4 判定代理). Their 「第二问」 is a different object.

Numbering note: this charter originally planned 066–069. 066 was taken by `charter-judge-agent-t4.md`. This batch is 070–073.
...
- 070 claims: if we open Q2's own cell, the person looking at the result sees another label; 065 answered who writes, not what the person sees. Attack: is 「标签」 so broad that 070 is tautological and just restates 060? If we admit 070, will readers hear 「B is approved」 and undo 065? Is there a see-layer where opening the cell still does not add a label?

- 071 claims: admitting 070 does not revive NF-only / 3-to-4 / NE. Attack: the user can only see one chip. After 070, isn't the honest place the chip they already look at? Is refusing A/C/D after admitting 「it's a new label」 more casuistry? Would a sibling they must click into fail the user's question 「用户怎么看到」?

- 072 claims: a seen label is not a Judge-filled label. Attack: the person experiences any word on the result as Judge's second conclusion. Is 「who writes」 the implementation detail the user already rejected as word games? If `_fulfillment_panel` is already shown as the judge card, does a derived sibling still feel like Judge output? Should 072 be not-actionable because it only restates 047 / 065?

- 073 claims: none of the four named slots can host as a whole sentence; the only leftover is a sibling seen-label; opening still stops at charter §4. Attack: after 070, is the honest named slot just B? Did this run fail to choose among A/B/C/D? Should 073 be escalate-to-project rather than another kill-list? If verifier smuggled a frontend column, a new schema field, a public Chinese name, or 「所以 B 批准了」 into the conclusion, reject that part.
