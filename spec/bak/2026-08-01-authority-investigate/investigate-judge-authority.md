...
class AuthorityAnalysis:
    ...
    # 本分析回答的判断问题（项目级，不是单字段级）。
    # 例如："当多个配置源对枚举值合法空间给出不同说法时，以什么为权威标准？"
    judgment_point: str
    ...
    # 支撑锚点验证的 ToolRequirement ID 列表。
    # 引用 InvestigationManifest.tool_requirements 中的 tool_id。
    # 当 verification_method 需要动态执行（如 ES 查询、代码回放）时，
    # 必须有对应的 ToolRequirement 声明验证能力。
    # 纯静态验证（如引用文档、对比配置文件内容）时可以为空。
    tool_requirement_ids: tuple[str, ...] = ()

    # 验证能力的结构化类别。不得通过解析 verification_method 自由文本推断。
    verification_mode: Literal["static", "dynamic"] = "static"
...
class SourceClaim:
    ...
    # 该信息源类别的身份标识。
    # 例如："field_enums_args.yaml（全量枚举配置）"、"ES 数据库实际存储值"、"保险行业标准术语"。
    source_label: str

    # 该源对判断点的具体说法。
    # 例如："定义每个枚举字段的合法值集合，是系统校验值合法性的依据"。
    claim: str
...
class CausalChain:
    ...
    # 系统代码如何加载和使用该源。
    # 例如："field_registry.py 加载 → normalize_field_value() 校验 → 注入 LLM prompt"。
...
class AuthorityAnchor:
    ...
    # 权威锚点的描述：真正的标准是什么（项目级结论）。
    # resolved 时必填。
    # 例如："field_enums 反映下游数据库实际存储值，是合法值空间的首要权威；
    #        value_mappings 的 target 必须指向 field_enums 中的值才有效。"
    description: str
...
判断点不是评估维度本身，而是维度在评估时反复遇到的**一类**事实问题。AuthorityAnalysis 回答的是项目级的"这类冲突的裁决原则是什么"，不是单个字段/单个值的"谁对谁错"。例如：

- 维度 "downstream-usability" 和 "intent-completeness" 共同依赖判断点 "当多个配置源对枚举值合法空间给出不同说法时，以什么为权威标准？"——这覆盖所有枚举字段的所有冲突，不是只回答某个字段的某个值；
- 维度 "downstream-usability" 依赖判断点 "查询条件形式不同但可能语义等价时，等价性的裁决标准是什么？"——这覆盖所有操作符/值形式差异，不是只回答某一对操作符；
- 维度 "intent-completeness" 依赖判断点 "用户口语表达到系统枚举值的语义映射，权威定义在哪里？"——这覆盖所有口语映射场景。

同一个判断点可以被多个维度引用。具体的字段冲突（如 20 个 value_mappings vs field_enums 冲突）是支撑项目级结论的**证据**，不是独立的 AuthorityAnalysis。
...
```json
{
  ...
  "tool_requirement_ids": [
    "client_search.es_field_aggregation"
  ],
...
```json
{
  ...
  "tool_requirement_ids": [
    "client_search.es_query_comparison"
  ],
  "verification_mode": "dynamic"
}
```

### 1.5.6 `tool_requirement_ids`：权威验证与工具调用的关联

`AuthorityAnchor.verification_method` 描述"如何验证锚点"，但验证往往需要动态执行（ES 查询、代码回放、API 调用），不能只靠静态文本。`tool_requirement_ids` 把验证需求连接到 `InvestigationManifest.tool_requirements` 中已声明的 `ToolRequirement`，形成闭环：

```text
AuthorityAnalysis
  └── anchor.verification_method: "ES aggregation 查询各字段实际值分布"
  └── tool_requirement_ids: ["client_search.es_field_aggregation"]
                                    │
                                    ▼
InvestigationManifest.tool_requirements
  └── ToolRequirement(
        tool_id="client_search.es_field_aggregation",
        description="对指定 ES 索引的指定字段执行 terms aggregation，返回实际值分布",
        parameters={"index": "str", "field": "str", "size": "int"},
        result_interpretation="返回值集合与 field_enums 对比，确认枚举完整性",
        implementation=ToolImplementationRef(...) 或 None
      )
```

规则：

- `verification_method` 需要动态执行时，`tool_requirement_ids` 不得为空，必须指向有效的 `ToolRequirement`；
- `verification_method` 是纯静态比对（如"对比 template 和 protocols 文本"）时，`tool_requirement_ids` 可以为空；
- `verification_mode` 必须为 `static` 或 `dynamic`；`dynamic` 必须引用至少一个 ToolRequirement，`static` 不得引用 ToolRequirement；
- 一个 ToolRequirement 可以被多个 AuthorityAnalysis 引用（如 ES aggregation 工具同时服务枚举权威和语义映射权威）；
- ToolRequirement 的实现状态（已有 vs 待 Solidify 构建）由 `InvestigationManifest` 管理，AuthorityAnalysis 只声明依赖。

> **举例（client_search）**：
>
> `enum-value-authority` 的 verification_method 包含"ES aggregation 查询各字段实际值分布"→ 需要 `tool_requirement_ids: ["client_search.es_field_aggregation"]`。
>
> `evaluation-boundary-authority` 的 verification_method 是"对比 template 和 protocols 文本"→ 纯静态比对，`tool_requirement_ids` 为空。
>
> `semantic-mapping-authority` 的 verification_method 包含"追踪 normalize_query() 和 normalize_field_value() 的代码路径"→ 需要 `tool_requirement_ids: ["client_search.code_path_trace"]`（或调查者已通过静态代码分析完成验证，登记为 evidence_ref，则不需要工具）。
...
1. **resolved 锚点 + tool_requirement_ids** → Solidify 根据 ToolRequirement 构建或复用 VerifiableTool，使锚点的验证能力可执行；
2. **resolved 锚点 + evidence_ref_ids** → Solidify 从 evidence artifact 取得原始材料，固化为 ContextUnit；
3. **resolved 锚点 + verification_method（纯静态）** → Solidify 将验证逻辑固化为 ContextUnit 中的判断规则；
4. **unresolved 锚点 + unresolved_question** → Solidify 知道该判断点尚无权威结论，对应的维度能力需要保守处理。

ToolRequirement 到 VerifiableTool 的映射：

```text
AuthorityAnalysis[enum-value-authority]
  ├── tool_requirement_ids: ["client_search.es_field_aggregation"]
  │         │
  │         ▼
  │   ToolRequirement(tool_id="client_search.es_field_aggregation",
  │                   implementation=ToolImplementationRef(
  │                     module_path="../tools/es_aggregation.py",
  │                     factory="build_es_aggregation_tool"))
  │         │
  │         ▼  Solidify
  │   VerifiableTool(execute_fn=build_es_aggregation_tool())
  │
  └── evidence_ref_ids: ["authority-conflicts-scan-20", ...]
            │
            ▼  Solidify
      ContextUnit(content=evidence artifact 原文)
```

> **举例（client_search）**：
>
> 1. `enum-value-authority`：tool_requirement_ids 指向 ES aggregation 工具 → Solidify 构建 VerifiableTool，使 Judge 能动态验证枚举完整性；evidence_ref_ids 指向 20 个冲突的扫描结果 → 固化为 ContextUnit 提供冲突分类知识。
>
> 2. `evaluation-boundary-authority`：无 tool_requirement_ids（纯静态比对）；evidence_ref_ids 指向 judge_boundary_template 原文 → 固化为 ContextUnit。
>
> 3. 若某锚点 unresolved → Solidify 不构建任何 Tool 或 ContextUnit，对应维度标记 not_evaluable。

Solidify 不得：
- 把 AuthorityAnalysis 的完整 JSON 直接注入 prompt；
- 用 AI 生成的摘要替代 evidence artifact 的原始内容；
- 忽略 unresolved 状态，把未确认的锚点当作已确认使用；
- 在没有 ToolRequirement 声明的情况下自行构建验证 Tool。
...
- `tool_requirement_ids` 必须引用 InvestigationManifest.tool_requirements 中有效的 tool_id；
- 当 `verification_method` 描述了动态执行验证（如数据库查询、API 调用、代码回放）时，`tool_requirement_ids` 不得为空。
- `verification_mode` 必须与 ToolRequirement 依赖一致：`dynamic` 至少引用一个 ToolRequirement，`static` 不得引用 ToolRequirement。
...
## 2.10 本轮前置调查落地后的运行时边界

本次改造将“项目级调查”和“case 级判断”明确分层：

- `AuthorityAnalysis` 仍完整保存在 Judge 调查合同中，用于 Investigate 审计、Solidify 重新编译和项目变化后的重调查；
- Solidify/runtime 只投影 `business_expectations`、`live_boundary`、`evaluation_dimensions`，以及按 dimension 作用域编译出的 `authority_anchors`；
- `authority_anchors` 只保留 `analysis_id`、适用 dimension、resolved/unresolved 状态、结论、验证接口声明和保守运行时指令，不携带 `source_claims`、`causal_chain`、`evidence_ref_ids` 或 `causal_reasoning`；
- 当 `resolved` 锚点声明了动态 `tool_requirement_ids`，但本次 Solidify 没有可执行的已声明 Tool 时，投影必须同时列出不可用 Tool，并使用 `apply_resolved_anchor_but_do_not_assume_unverified_dynamic_facts`；这表示可以使用已经被项目级调查证明的稳定规则，但不能把尚未验证的 case 级动态事实当成已验证。`unresolved` 锚点仍必须使用 `not_evaluable_when_authority_is_required`，不得以静态结论补齐缺口；
- 每个 `authority:<analysis_id>` 必须拥有独立的 Solidify mapping 和 runtime observable。一个观测可以覆盖多个公共资产，但不得把多个 AuthorityAnalysis 合并成一个不可区分的 authority source；观测证据必须能逐项证明 anchor 存在、dimension scope 保留以及调查过程字段未泄漏；
- runtime 不再读取 `authority-registry.json`，也不再按全局 Tier 或 `conflict_resolution_matrix` 进行裁决；该文件只保留为历史调查材料，直到新的固化资产完成 Current/Draft 回归后再删除；
- `authority:<analysis_id>` 必须进入 Solidify receipt 的 `required_source_ids`，并且由一个可观测的 Context/Tool 投影覆盖，防止 AuthorityAnalysis 只是“写进调查包但未被执行”；
- `evidence_ref_ids` 和 `tool_requirement_ids` 在 InvestigationManifest 校验阶段交叉验证，不能只在孤立的 Judge contract validator 中通过；
- runtime tool 只允许执行调查阶段定义好的封闭式事实读取或比较，不允许以 tool 为入口重新进行开放式来源调查。

这意味着当前迁移不是删除旧文件后让模型自由判断，而是：

```text
AuthorityAnalysis（项目级调查）
    → Solidify authority_anchors（最小运行时投影）
    → ContextUnit / 已声明事实 Tool
    → Judge case-time 判断
```

在新的固化资产完成等价性和无退化回归之前，旧 `authority-registry.json` 不作为运行时真相源，但仍可作为 Current 行为的审计材料保留。
