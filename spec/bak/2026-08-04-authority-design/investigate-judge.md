...
class JudgeInvestigationContract:
    ...
    # 权威分析：对关键判断点的权威来源进行因果链追溯。
    # 可选，向后兼容。详见 spec/alg/investigate-judge-authority.md。
    authority_analyses: tuple[AuthorityAnalysis, ...] = ()
...
### 3.1 权威分析扩展（AuthorityAnalysis）

当多个官方项目材料对同一判断点给出不同说法时，Judge 需要追溯每个信息源的因果链来确定真正的权威锚点。`authority_analyses` 是 `JudgeInvestigationContract` 的可选顶层对象，每个 `AuthorityAnalysis` 回答一个项目级判断点的权威问题。

完整协议见 `spec/alg/investigate-judge-authority.md`，包括：
- `AuthorityAnalysis`、`SourceClaim`、`CausalChain`、`AuthorityAnchor` 的 Dataclass 定义；
- 因果链追溯、事实验证、业务方澄清三种互补的权威确认方式；
- Validator 门禁（至少 2 源、因果链完整性、锚点状态一致性、priority-only 语义检查）；
- 与 `InvestigationManifest.evidence_refs` 和 `tool_requirements` 的引用关系。
...
当前 Case 原子验收项
  ↓ 若该评价点依赖项目级 Authority
按 finding_id 引用 SolidifiedAuthorityAnchor
```

Planning 必须在读取 Live actual 前完成，且不得：

- 比较原始 Authority 资料；
- 创建新的 AuthorityFinding；
- 把 `unresolved` 改成 `resolved`；
- 因为预见 actual 的表现而增删维度、验收项或改变 `blocking`。

当前 Case 验收项可通过 Draft 内部字段引用 Authority：

```text
CaseEvaluationPoint.authority_finding_ids[]
  → SolidifiedAuthorityAnchor.finding_id
  → AuthorityInvestigationReport.findings[finding_id]
  → basis_source_ref_ids
  → EvidenceRef
```

该字段只保存已固化 Finding 的 ID。Core 必须在当前固化资产中解析到对应
`SolidifiedAuthorityAnchor`；Planning 不复制资料正文、调查推理或 Anchor
内容，也不直接读取完整 Authority Report。Finding 和 Anchor 的生产、状态及
来源回溯由
`spec/alg/investigate-judge-authority-optimize.md` 定义。

### 5.2 Authority Gate：根据当前信息决定能否评价

Planning 负责表达“这个评价点依赖哪些 Authority”；确定性 Gate 负责检查这些
Authority 当前是否足以支持判断：

```text
评价点不依赖 Authority
  → 按现有三态规则评价

评价点只引用 current_behavior
  → 可解释当前实现，但不能据此证明正式业务正确

评价点依赖 normative_rule / external_fact
  ├─ 全部 resolved   → Judge 可依据固化结论评价
  └─ 任一 unresolved → 该评价点必须 not_evaluable
```

Gate 必须校验：

- Finding 来自当前 `InvestigationSnapshot` 和当前 project；
- 当前评价点的 `evaluation_dimension_id` 包含在 Finding 的
  `dimension_ids`；
- 每个 `authority_finding_id` 都能解析到当前固化 Authority 资产中的 Anchor；
- `current_behavior` 不得替代缺失的 `normative_rule` 或 `external_fact`。
...
人类补充资料后，必须重新 Investigate 并生成新的 snapshot；只有新的
AuthorityFinding 变为 resolved，后续 Case 才能解除 Gate。Planning 和 Judge
本身不能用临时提示绕过 unresolved。
...
10. Planning 对 BusinessExpectation、EvaluationDimension、LiveBoundary 和
    AuthorityFinding 的选择边界尚未形成稳定合同；
...
- 允许评价点只通过 `authority_finding_ids` 引用已固化 Finding；
- 确定性校验 Finding 的 snapshot、project 和 EvaluationDimension；
- unresolved 的 normative_rule 或 external_fact 必须把对应评价点约束为
  `not_evaluable`；
- Judge Summary 必须输出 unresolved 原因、待补资料和依据来源；
- 增加“无 Authority 依赖”“current_behavior 仅解释现状”“resolved 可评价”
...
- Planning 在观察 actual 前完成，且不重新调查或修改 AuthorityFinding；
