...
字段定义、规范规则（normative_rule）或外部事实（external_fact）。调查阶段要主动
...
`unresolved` + `required_evidence` 缺料清单 → 该清单与调查侧 unresolved Finding 的
`required_evidence` 合并为下一轮调查的输入。新一轮调查按本条循环补证，成功补到决定性
资料后固化为 resolved 配对；Runtime 不自行补证，也不把 unresolved 当永久结论。
...
- 第二层是兜底：任何 `decision_question` 都可以现场综合，但证据不足时只能返回
  unresolved；
- 两层共享同一份资料空间：第二层现场综合用的就是第一层调查报告物化后的原始资料
  （EvidenceSpace），不是另一套资料；Authority Agent 只在绑定空间内 search/load，
  缺料就返回 unresolved，不主动出去补证据（`authority.md` §8.5）；
