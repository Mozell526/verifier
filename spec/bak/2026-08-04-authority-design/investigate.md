...
这里的“协议运行”与“Loop 判断”必须严格分离。`run_iteration.py` 一类代码可以预校验冻结 case、检查运行依赖、调用 Current/Draft、逐 case 原子保存结果与异常；它不能解释业务结果、决定候选是否更好或自动修改候选。调查是否充分、缺失验证应返回 Investigate 还是 Solidify、Draft 是否满足 objective/review，均由 Draft Skill 驱动的 Harness AI 判断并记录事实指针。
...
- `module_path` 保存稳定的、相对项目目录的 production 逻辑路径；若当前运行 Draft，公共 `RoleAssetMapping` 必须将该 production path 解析到已声明的 `candidate_path`，validator 和 runtime 都使用解析后的实际文件；
- `factory` 必须可导入和调用；
...
不得把 `draft/tools/...` 物理路径固化进 manifest。否则 Promote 纯搬运后引用会失效，并迫使晋升阶段改写调查产物。稳定 production 逻辑路径加 `RoleAssetMapping` 重定向保证同一 manifest 可同时服务 Draft 验证和晋升后的 Production。
...
docs/traces/<flow>.md    # 使用方法、操作索引、节点证据、验证路径和观察边界
```
...
- 从哪些 RunTrace/Judge gap 观察开始：
- 如何把观察映射到候选节点/边：
- 如何选择最小决定性验证，以及何时 unresolved：

## Operational index
| Node | Enter when / trace signal | Input → output | Observe or verify | Supports | Boundary / next step |
|---|---|---|---|---|---|

## Investigation procedure
1. 从当前 gap 和业务边界开始；
2. 沿操作索引深度优先选择节点；
3. 执行 Tool/replay/probe 并按结果进入下一节点；
4. Finalization 重载决定性材料，缺少必需观察时 unresolved；
...
`docs/traces/` 是 Attribute 专属的固定文件形状，不是公共目录能力。图和配套文档用于表达“业务请求怎样经过关键串联点传播”并指导 runtime 选择验证路径，而不是代替来源或验证：节点和边必须引用 manifest 中的 `EvidenceRef`，需要执行确认的关系必须关联 `ToolRequirement`。操作索引必须能从可观察 trace 信号导航到节点、当前事实获取方法、结果分支和停止边界；只有节点名称与关系线不合格。无法取得业务内部 trace 时，必须明确静态调用关系只能证明结构，不能伪装成真实执行路径。
...
Judge 涉及“多份业务资料对同一判断点说法不一致”或“外部验收标准能否唯一决定结论”时，按 `spec/alg/investigate-authority-judge.md` 做权威调查：以 `materials` 为第一组织维度，逐份资料声明它在什么 `conclusion_kind + scenario + conditions` 组合内直接决定什么（`governs`），`related_to` 只列仅相关事项；调查结论综合为 `findings`（resolved/unresolved，unresolved 必须带 `unresolved_reason` 与 `required_evidence`）。Judge 契约含 `authority_analyses` 时，必须登记 `docs/authority-investigation-report.json`（结构化真相源）及其确定性渲染 `docs/authority-investigation-report.md`，并分别以 `judge_authority_investigation_report_json` / `judge_authority_investigation_report_markdown` 登记到 Manifest `artifact_refs`。报告是导航摘要，不复制文件内容、不含任何 case 的 actual/score/verdict；物化后的原始资料才可进入运行时 `basis_evidence_ref_ids`。权威调查的运行时综合与 Gate 规则见 `spec/alg/authority.md`。
...
Mock 没有强制文件名。场景空间可以在 `overview.md` 说明；只有内容复杂时，才通过 `artifacts` 登记 `docs/input-contract.md`、`docs/scenario-space.md` 等文件。Solidify 将稳定的输入契约、场景约束和业务边界注册为 Mock 可见的 mandatory ContextUnit，并在生成前确定性装载；baseline 不要求 Mock Agent 自行 Search/Load。Context policy 必须阻止 Mock 读取 promotion-only unseen case、样本答案和 case 专属修复信息。需要执行验证的能力仍固化为 Tool，候选 `draft/mock.py` 使用同一批 Context/Tool；不得把具体迭代 case 答案固化为生成规则。
...
/draft solidify：Harness AI 按协议实现候选
  ...
  在公共 role_assets 中声明 enabled / roles / production_path / candidate_path
...
用户确认 /draft promote：verifier 协议代码
  读取 project.yaml 中运行时已经使用的 Role/asset 映射
  preflight → 确定性 move/copy → 关闭对应 draft switch
```
...
ContextUnit 的 `name/description` 必须让 Role 或确定性 loader 能判断其用途，并带上与现有权限模型一致的 Role 可见性。调查资料用于导航和判断，不因被加载就自动证明当前 case 的结论。

ContextUnit 接入分成两个既有层次，不增加 Role 专属 Context schema：

1. 公共注册与访问层：所有 Role 共用现有 `build_context_runtime`、`ContextPolicyResolver`、Context registry、source/hash、`content_ref`、预算和审计机制；`role_assets.roles`、ContextUnit 自身 `roles` 与 Context policy 必须一致。本文中的 `ContextRun` 指现有 runtime 为单次 Role 执行创建的、已经绑定 Role policy 和审计状态的访问句柄，不是新增 schema。
2. Role 消费层：ROLE.md 和候选 Role 决定在何时、以何种方式消费允许访问的 ContextUnit。该差异属于算法和 policy 配置，不新增 `JudgeContext`、`MockContext` 或统一的 Evidence/Finalization 流程。

baseline 的消费方式固定如下：

| Role | ContextUnit 消费方式 | 明确不继承的行为 |
|---|---|---|
| Attribute | 保留现有交互式 Search/Load；ToolResult 可注册为当前 case 的动态 ContextUnit；Finalization 重载最终 evidence，Reviewer 审核被引用全文 | 无 |
| Judge | 业务契约、验收边界等稳定资料通过现有 policy 的 `mandatory_ids` 声明，运行前调用 `load_mandatory_context_units()` 确定性注入 | 不开放 Agent Search/Load；不引入 EvidenceRef Finalization、Attribute Reviewer 或内部系统探索 |
| Mock | 输入契约、场景约束等稳定资料通过 `mandatory_ids` 声明，生成前确定性注入 | 不开放 Agent Search/Load；不读取 promotion-only unseen case、答案或 case 专属修复信息 |
| Live 或新增 Role | 由 ROLE.md 在上述两种既有方式中选择，必要时组合，但必须声明材料边界和失败语义 | 不自动继承 Attribute 的证据闭环 |

Judge/Mock 采用 mandatory 注入不是把全部调查包塞入 Prompt。Solidify 只把运行必需且体量受预算约束的单元加入 `mandatory_ids`；导航材料、大型文档和仅在少数条件需要的资料保持为非 mandatory ContextUnit。只有当真实项目证明资料规模或条件分支已无法由 mandatory 集合稳定承载时，才允许 Judge/Mock 在同一个 `ContextRun` 上增加受控检索；这项演进不能修改 JudgeResult/MockResult 等公开 Role schema。
...
- 同时在公共 `role_assets` 中登记稳定 asset ID、Role 权限、正式目标和候选位置；Production/Draft 不各建一套 Tool 开关；
...
结构 validator 通过只证明调查包可读、引用可追溯和已声明实现可装载，不证明调查足以支持 objective。Solidify 开始前，Harness AI 必须逐项回看 Manifest：如果当前目标依赖某个仍有 `implementation_gap` 的 ToolRequirement，应先把它实现为真实验证能力，或承认该缺口使候选只能 unresolved 并路由回 Investigate；不能绕开缺口，以无关 probe 或并不存在的项目私有接口组织候选。固化完成后还必须用真实 project loader 验证 Tool 注册/执行、ContextUnit 注册/加载和候选 Role 实例化，并建立“调查 requirement → 固化文件 → runtime audit”的可复查对应关系。该对应关系使用现有 Manifest、文件和运行记录，不新增公共阶段 schema。
...
- 在任何 Role LLM 调用前一次性校验全部 iteration case；Attribute 还必须预检 Context Search 和动态证据注册依赖，并在 Current/Draft 每侧 Role 返回后检查该次运行审计；预检通过不代表长运行期间依赖持续健康；
...
- 逐 case 原子保存两侧原始 RoleResult、异常、Context Search/Load、Tool Call、runtime Reviewer 决策和可取得的耗时/用量；中途失败不得丢失已完成事实；
- Context Search/Load 的网络、provider、配置或内容解析基础设施异常，动态 evidence 注册失败，或 runtime Reviewer 基础设施失败时，本轮必须标记 `failed`，不得进入 `improved`；ContextUnit ID 不存在、权限/预算超限和参数错误作为 Role 请求行为事实保留，由 Harness AI 判断其对候选质量的影响；
- 不输出通用“draft 更优”结论；
...
`/draft promote` 属于 verifier 协议代码，必须在用户明确确认后执行。AI 只生成 Review 和 promotion 建议；文件选择、目标路径和 move/copy 行为必须在 Draft 构建和运行前已经写入项目配置。Promote 不调用 LLM、不扫描 import 推断依赖、不生成 promotion manifest、不改写被搬运资产的内容；它只运行确定性文件操作，并更新已经定义好的 switch/path 控制字段。

### Production、Draft 与 Promote 共用 Role 资产配置

保留现有 `project.yaml` 的 `<role>_draft.enabled + module`，它只切换 Role 实现文件，例如 `attribute.py ↔ draft/attribute.py`。Tool、Context、调查资料及其他辅助能力不再定义成“Draft 资产”；项目使用一个环境无关的 `role_assets` 列表，统一控制 Production 和 Draft 中每个 Role 可以使用哪些能力：

```yaml
attribute_draft:
  enabled: true
  module: draft/attribute.py
  reason: "validate candidate attribution"
...
role_assets:
  - asset_id: condition_replay
    kind: tool
    enabled: true
    roles: [attribute]
    production_path: tools/condition_replay.py
    candidate_path: draft/tools/condition_replay.py
    replace: false

  - asset_id: business_schema_check
    kind: tool
    enabled: true
    roles: [attribute, judge, mock]
    production_path: tools/business_schema_check.py
    candidate_path: draft/tools/business_schema_check.py
    replace: false

  - asset_id: attribute_project_map
    kind: context
    enabled: true
    roles: [attribute]
...
- Production loader、Draft loader、Context builder 和 Tool registry 都使用同一个 `enabled + roles` 过滤规则，不存在另一套 Draft Tool 开关；
- Role 使用 Production 实现时解析 `production_path`；使用 Draft 实现时优先解析 `candidate_path`，未配置 candidate 的稳定资产继续使用 production path；
- Promote 脚本对本次晋升 Role 使用、且声明了 `candidate_path` 的同一组资产执行搬运；`production_path` 始终是确定目标。

`enabled` 和 `roles` 不因 Production/Draft 切换而变化；两侧的权限和开关语义完全一致，差异只来自相同 asset ID 当前是否已有 production/candidate 实现。稳定能力可以不配置 `candidate_path`；全新候选能力仍必须预先声明将来的 `production_path`，但该正式文件可以尚不存在，此时 Current 必须明确记录该能力 unavailable，Draft 才能使用 candidate，不能伪装成两侧能力相同。若声明了 candidate path，Draft 开启时候选文件缺失必须报错，不能静默回退到 production 版本。候选 Role 直接依赖了未登记资产，或配置把资产授权给不允许使用该材料的 Role 时，Draft check 必须失败。

公共配置解析应收敛为一个确定性函数，例如 `resolve_role_assets(spec, role, use_candidate)`；Production/Draft loader、Draft check 和 Promote 必须调用它，不得分别实现多套工具开关和路径规则。现有 `ToolImplementationRef`、Context builder 和 ContextUnit 协议继续负责“怎样加载”；`RoleAssetMapping` 只负责“是否启用、哪些 Role 可用、两种实现位置在哪里”，不重复 Tool factory 或 Context 内容 schema：

```python
@dataclass(frozen=True)
class RoleAssetMapping:
    asset_id: str
    kind: str                   # tool / context / investigation / other
    enabled: bool
    roles: list[str]            # Production 与 Draft 共用的 Role 权限
    production_path: str        # 正式目标必须预先确定；文件可以尚不存在
    candidate_path: str = ""    # 没有候选实现时，两侧均使用 production_path
    replace: bool = False       # 正式目标已存在且内容不同时是否允许整体替换
```

`RoleAssetMapping` 是项目能力选择和物理位置映射，不承载生成逻辑、Review 结论或文件内容。`module` 继续由现有 `<role>_draft` 配置表达，晋升目标固定为项目根目录 `<role>.py`，不在 `role_assets` 重复登记。若 ContextUnit 自身也声明 `roles`，validator 必须确认它与这里的权限一致，不能出现配置允许但 Context policy 拒绝的双重真相源。

### 启动服务时切换 Production / Draft

服务继续直接使用现有 `project.yaml` 开关，不新增启动 profile、状态 API 或前端切换协议。需要查看某个 Role 的 Draft 效果时，修改对应项目配置：

```yaml
# Draft：加载 impl/projects/<project>/draft/attribute.py
attribute_draft:
  enabled: true
  module: draft/attribute.py

# Production：加载 impl/projects/<project>/attribute.py
attribute_draft:
  enabled: false
  module: draft/attribute.py
```

`judge_draft`、`mock_draft` 等 Role 使用相同规则。不同 Role 可以独立切换；例如只打开 `attribute_draft.enabled` 时，Judge 仍走 Production，Attribute 走 Draft。

配置完成后按现有入口启动或重启服务：

```text
bash run.sh server
bash run.sh server --port 8022
```

因此最小查看流程是：设为 `false` 启动并保存 Production 结果，设为 `true` 重启同一服务并运行相同输入，再比较结果。切换不移动文件，也不等于 Promote；只有 `/draft promote` 才会把已验证候选搬到正式目录并关闭 Draft 开关。

Role 切换只决定 `<role>.py` 或 `draft/<role>.py`。两种实现使用同一套 `role_assets.enabled + roles` 能力开关；resolver 根据当前 Role 是否启用 Draft 选择 production/candidate path，不为服务启动再建立一套 Tool/Context 配置。
...
```text
python -m impl.cli draft-promote --project <project> --role <role> --check
python -m impl.cli draft-promote --project <project> --role <role> --apply
```

`--check` 只解析并打印配置中已经存在的操作，不写文件、不生成 manifest。`--apply` 必须按固定顺序执行：

1. 确认用户显式提供 `--apply`，且该 Role 已处于 ready-to-promote；
2. 通过公共解析函数取得 `<role>_draft.module` 和该 Role 在 Production/Draft 中共用的 `role_assets`；
3. 检查所有 candidate/production path 均在当前项目内，candidate 位于 `draft/`，production 不位于 `draft/`，无重复或祖先/子路径冲突；
4. 重新运行协议加载、promotion-only regression、依赖完整性和目标冲突检查；任一失败则零文件变更；
5. 将候选 module 整体移动到正式 `<role>.py`；
6. 对每个存在 `candidate_path` 的 asset：若 candidate 文件不再被其他仍启用 Draft 实现的 Role 使用则移动到 `production_path`，并将该 asset 的 `candidate_path` 清空；否则复制并保留 candidate 文件与路径；
7. 目标不存在时直接落位；目标存在且内容不同时，只有 `replace: true` 才允许整体替换，永不合并或改写内容；
8. 将本次 Role 的 `<role>_draft.enabled` 确定性改为 `false`，重新按 production 路径加载并运行最小回归；
9. 任一步失败，使用临时事务备份恢复文件和 `project.yaml`；成功后删除临时备份，不在项目内留下新产物。

Promote 的最小不变量是：候选运行时加载过的代码和资产，与正式目录最终得到的字节完全一致；Promotion 前后同一 Role 的 `enabled + roles` 策略也必须一致，候选实现搬到 production path 后由同一开关自然接管。脚本对所有路径映射采用同一套 move/copy 逻辑，不因 `kind`、Tool/Context 或“是不是 Draft 资产”改变搬运算法。它不能渲染模板、重写 import、生成 wrapper、补 Tool、修改调查文档或临场选择额外文件。若某项不能直接搬运，必须回到 Solidify 先把 candidate/production 形状整理一致，再重新运行 Draft Loop。

最少映射为：

```text
<role>_draft.module                                      → <role>.py              # move
role_assets[candidate_path，仅当前 Draft 消费者]         → production_path        # move
role_assets[candidate_path，仍有其他 Draft 消费者]       → production_path        # copy
```
...
若某项事实或 Tool 应被多个 Role 使用，必须在 `role_assets.roles` 中声明；Production 和 Draft 都遵守相同权限，不能通过让一个 Role 直接读取另一个 Role 的候选调查包实现共享。共享资产仍有其他启用中的 Draft 消费者时必须 copy，不能 move 后破坏其运行。
...
| 只有 Attribute 完整接入 ContextUnit runtime，Judge/Mock 仍直接加载项目文档 | 三个 Role 共用 ContextUnit 注册、权限和审计层；Attribute 交互式 Search/Load，Judge/Mock baseline 使用 mandatory 确定性注入 | 消除 Solidify 产物对 Judge/Mock 不可消费的断点，同时不把 Attribute 的复杂证据协议强塞给其他 Role |
...
| Draft loader 已能按 `<role>_draft` 切换 Role 文件，但 Production/Draft 的 Tool/Context 开关和 Promote 映射未统一 | `<role>_draft` 只切 Role 实现；公共 `role_assets` 同时驱动两侧能力选择、检查和确定性搬运 | 两侧使用同一开关与权限语义，Promote 不再临场扫描、生成清单或猜共享关系 |
| Draft/Production 服务切换方法没有在 Investigate 流程中说明 | 继续使用现有 `<role>_draft.enabled`，修改后启动或重启服务 | 无需新增后端参数、状态 API 或前端开关 |
...
- 在项目配置 schema 增加 `RoleAssetMapping`，并为 `attribute/judge/mock` 提供统一的 Role switch 与资产解析入口；
...
- Attribute 的 Harness 语义审查还必须确认链路展开到 objective 所需的实际分支、匹配/捕获/转换和后处理节点；仅有通用入口/路由/输出图不得视为调查收敛。同名 Markdown 必须在逐节点说明之外提供 `How to use this trace map`、`Operational index`、`Investigation procedure`，把当前 trace 观察连接到候选节点、验证 Tool/EvidenceRef、结果分支与 unresolved 停止边界。它是既有 artifact 的消费契约，不新增公共 schema；validator 只检查结构存在，内容是否足以导航当前 objective 仍由 Harness AI 审核；
- 检查 EvidenceRef 可追溯并带 source revision/hash；
- 导入每个 Tool `module_path + factory`；
- 确认 factory 返回 `VerifiableTool`；
- 对 Tool 执行项目提供的最小真实测试；测试参数由检查命令通过 `--tool-inputs <JSON或文件>` 显式传入，不写入 Manifest 或 `ToolRequirement`；
- `--execute-tools` 开启时，每个已实现 Tool 都必须真实执行：无参数 Tool 至少空参执行一次，有参数 Tool 至少提供一组 kwargs；缺少输入、未调用、抛出异常，或返回 `inconclusive/failed/error` 都必须非零退出，不能返回 `ok: true`；
- 输出错误并阻断 solidify，不通过 fallback 保留伪产物。

### Task 5：接入公共 ContextUnit 消费（P0）

- 增加 `draft/context_builders/<role>_investigation.py` 的项目模板；
- 将 overview 和 artifacts 按主题注册为独立 ContextUnit 或受控资源；
- 让 Attribute、Judge、Mock 通过现有 `build_context_runtime` 和 `ContextPolicyResolver` 建立 Role 有界的 `ContextRun`，不得再为 Judge/Mock 建平行 Context store 或专属 Context schema；
- Production/Draft Context loader 使用同一个 `role_assets.enabled + roles` 选择资产，并校验 ContextUnit 自身 roles 与配置一致；
...
- Attribute 继续使用现有 Search/Load、动态 ToolResult 注册、Finalization 和 Reviewer 链路，不因公共接入而降级；
- Judge 将稳定业务契约和验收边界配置为 `mandatory_ids`，在 Judge LLM 调用前通过 `load_mandatory_context_units()` 确定性注入；baseline 不给 Judge Agent 暴露 Search/Load；
- Mock 将输入契约和场景约束配置为 `mandatory_ids`，在生成前确定性注入；baseline 不给 Mock Agent 暴露 Search/Load；
- 将 Judge/Mock 目前直接读取的项目文档迁移为有稳定 ID、来源和 Role 权限的 ContextUnit；完成迁移后删除同一材料的直接加载旁路；
- 缺失、越权或无法解析的 mandatory ContextUnit 必须阻断运行并暴露原因，不得退回直接读文件或空上下文；
- 当前 case 输入、业务输出和 reference 保持显式 Role 输入，不注册成跨 case 静态 ContextUnit；
...
- Production/Draft Tool loader 先按同一个 `role_assets.enabled + roles` 选择 asset ID，再根据当前实现位置和已校验的 ToolImplementationRef 装配 Tool；
...
- Solidify 必须逐项处理 objective 相关的 implementation gap；如果候选改善依赖该能力却没有补实现，Draft check/Review 必须失败并路由回 Investigate 或 Solidify；
...
- 正式运行前预校验全部冻结 case 和必需基础设施；逐 case 原子落盘，失败报告保留部分事实；
...
- 扩展 `project.yaml` 模板：现有 `<role>_draft.enabled/module` 只控制 Role 实现切换；公共 `role_assets` 以 enabled、roles、production_path、candidate_path 和 replace 同时控制 Production/Draft 能力；
- 实现 `resolve_role_assets(spec, role, use_candidate)`，让 Production/Draft loader、check 和 Promote 共用同一份能力选择结果；
- 实现 `impl/core/draft_promotion.py` 和 `python -m impl.cli draft-promote --check/--apply`；
...
- `--execute-tools` 缺少必需 smoke inputs、Tool 返回非成功状态及未知 tool_id 时的失败测试；
- RoleAssetMapping 路径逃逸、重复 production path、Role 权限、共用开关语义、candidate-only availability、replace 冲突和共享消费者测试；
- Draft/current Context 和 Tool 隔离测试；
- 不同 Role 候选调查包、Context 和 Tool 权限隔离测试；
- ContextUnit 按需加载与大文档预算测试；
- Attribute/Judge/Mock 使用同一 Context registry、policy resolver 和审计机制的接入测试；
- Judge/Mock mandatory ContextUnit 的确定性装载、稳定顺序、预算、缺失、解析失败和越权阻断测试；
- Judge 不可见内部源码、内部 trace 和 Attribute 候选材料，Mock 不可见 promotion-only unseen case、答案和 case 专属修复信息的权限测试；
- Judge/Mock 禁用 Agent Search/Load 时仍能通过 mandatory Context 完成 baseline，且不触发 Attribute EvidenceRef/Finalization/Reviewer 的测试；
...
- `check_draft --promotion` 缺少 unseen、unseen 为空或 unseen runner 失败时必须非零退出；
- Promote `--check` 零写入、独占 move、共享 copy、禁用开关、字节一致性、部分失败全量恢复和禁止生成内容测试；
...
| `impl/core/schema/project.py` | `RoleAssetMapping` 与各 Role 共用能力配置边界 |
| `impl/core/project_loader.py` | Production/Draft loader、check、promote 共用的配置解析入口 |
| `impl/core/context/` | Attribute/Judge/Mock 共用的 Context 注册、Role policy、mandatory 装载、预算和审计基础设施 |
| `impl/core/attribute_environment.py` | Attribute 交互式 Search/Load、动态 ContextUnit 和证据闭环接缝 |
| `impl/core/judge.py`、`impl/core/mock_agent.py` | 在 Role LLM 前确定性装载各自 mandatory ContextUnit，不复制 Context runtime |
| `impl/core/draft_promotion.py` | 确定性 preflight、move/copy、配置切换和事务回滚 |
| `impl/cli.py` | `draft-promote --check/--apply` 正式协议命令 |
...
2. InvestigationManifest + ToolRequirement + ToolImplementationRef + RoleAssetMapping
3. 调查包模板与 validator
4. Role 契约与 Attribute Mermaid 校验
5. Context builder 固化
6. Tool loader 固化
7. Draft Loop：协议运行、多源评估、按 config.review 判断与修订串联
8. Production/Draft 共用 Role asset 配置、确定性 Promote 与一致回滚
