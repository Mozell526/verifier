...
`non_material_variations` 记录可以被观察到、但不改变本纬度业务结论的差异。它比“等价”更通用，可以表达语义等价、顺序差异、数值容差、合法结构变体和非确定性措辞。不存在已确认允许差异时可以为空；不得用它豁免会改变业务结果的真实偏差。
...
当前用户输入 / reference / 外部业务合同 / Live actual
  + 当前适用的 EvaluationDimension
  → 生成并判断当前 Case 的 BusinessExpectation

LiveBoundary
  → 约束所有 expectation 的 Live 归责范围

EvidencePolicy
  → 约束所有 expectation 的证据优先级和全局不可评估边界
...
4. 对每个纬度调查业务严重性和不影响结论的允许差异；
5. 从现有 `judge_boundary` 和权威业务资料中分别提炼 `LiveBoundary` 与 `EvidencePolicy`；
...
- 每个纬度都有具体 `evaluation_target`、`expected_result`、`materiality_rule`、required evidence 和三态条件；
- 每个纬度都显式提供 `non_material_variations`，允许为空元组；
...
6. validator 只验证公共 Manifest、EvidenceRef、ToolRequirement 和 artifact 路径，不验证本 spec 的开放类型和语义关系；
7. Solidify 没有强制证明候选实际消费评估纬度、`LiveBoundary` 和 `EvidencePolicy`；
...
- 校验 dimension ID、开放 type、materiality、EvidenceRef 和 ToolRequirement，不维护封闭类型枚举；
...
- 增加缺失强制 artifact、空纬度、materiality 缺失、三态缺失、未实现关键 Tool 和未消费资产的失败测试；
...
- validator 能阻止缺失、非法引用、空纬度、materiality 缺失和三态不完整，同时允许项目扩展纬度类型；
- 调查会默认考虑用户期望和下游可用性，但不强制生成不适用的固定纬度；
- `non_material_variations` 能稳定承接 client_search 的语义或结果集等价，也能表达其他项目的非实质差异；
