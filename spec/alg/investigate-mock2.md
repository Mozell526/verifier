...
class MockDemandSpace:
    ...
    # dimension_id → 为检验该维度，Mock 数据必须具备什么。
    evaluation_coverage_requirements: dict[str, str]
...
- `evaluation_coverage_requirements`：为了评估每个维度，数据必须包含什么；
- `variation_space`：同一需求空间内的具体用户现实可以怎样变化；
- `validity_constraints`：怎样避免生成无效、虚假或内部冲突的数据。

`evaluation_coverage_requirements` 的 key 必须是 `EvaluationDimension.dimension_id`，value 必须直接说明该需求空间中的 Mock 数据怎样使该维度可被检验。它不是 Judge 结论，也不是目标答案。
...
选择一个 MockDemandSpace
  ...
  → 满足各 evaluation_coverage_requirements
...
MockDemandSpace(
    ...
    evaluation_coverage_requirements={
...
- `evaluation_coverage_requirements` 的 key 引用真实 `dimension_id`；
...
- 审查生成数据是否满足各 `evaluation_coverage_requirements`，而非只通过 schema；
...
- 新增 `MockInvestigationContract`、`BusinessValue`、`EvaluationScope`、`EvaluationDimension` 和 `MockDemandSpace`；
