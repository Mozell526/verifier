...
默认使用固化 fixture。种子 case 从现有非流式项目的 live_request 转成信封，并带上 `capability_ref`。`scenario` 按被探测能力分桶，与 `capability_ref` 同名：`client_search` / `policy_search` / `marketting-planning-intent`。
