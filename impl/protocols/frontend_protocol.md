...
- Case-pool run status (`pending` / `running` / `done` / `error`) is not fulfillment status. `error` means this run did not produce a usable judgment (`llm_call_failed`, `llm_output_validation_failed`, or a chain/batch exception); a genuine `not_evaluable` fulfillment remains `done`.
