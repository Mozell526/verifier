...
def judge_trace(
    ...
    system = (
        ...
        "此时 interpretations 必须是空数组 []（这是正确缺省；绝大多数 expectation 应如此）。\n"
        "- 仅当某条 expectation 附加了超出诉求字面、且会改变对账结论的口径时，"
        "才往 interpretations 写入条目；每条对象只允许三键："
        "{\"statement\": 口径内容, \"warrant\": 材料引用或 null, \"divergent\": true/false}；"
        "禁止 interpretation/note/basis/spoken/normalized/含义 等别名键\n"
        "- warrant 必须指向可回溯材料（项目评估文档、受治理规则、调查产物、authority.resolve 调用）；"
        "常识、行业惯例、举一反三不构成担保；给不出担保就把 warrant 设为 null（不要编造）\n"
        "- warrant 为 null/空的口径 → 对应 assessment 必须判 not_evaluable，"
